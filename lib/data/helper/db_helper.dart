...
class DBHelper {
  ...
  /*Future<bool> doesPasswordMatched({required String password}) async {
    ...
    Future<bool> addExpense({required ExpenseModel expModel}) async {
        ...
        /*if (rowsEff > 0) {
          updateBalence(expModel: expModel);
    ...
    Future<bool> updateBalence({required ExpenseModel expModel}) async{

      Database dbReference = await initDB();

      SharedPreferences myPref = await SharedPreferences.getInstance();
      int userId = myPref.getInt(column_user_id) ?? 0;

      var data = await dbReference.query(
          table_user,
          where: "$column_user_balance = ?",
          whereArgs: [userId]);
      ...
      if(expModel.type == 0){
        // Debit
        balance -= expModel.amount;
      }
      else{
        // Credit
        balance -= expModel.amount;
      }
      int rowsEff = await dbReference.update(table_user, {
        column_user_balance: balance
      },
          where: "$column_user_balance = ?",
          whereArgs: [userId]);
